...
const calcPOCandVAL = (data, rows = config.VP_ROWS) => {
  ...
  const workingBars = sessionBars.length >= 8 ? sessionBars : data.slice(-24);
...
const detectRejection = (candles, zoneLow, zoneHigh, direction) => {
  ...
  if (direction === 'BUY') {
    ...
    // 2. Bullish engulfing
    if (c.close > c.open && c.close >= p.high && c.open <= p.low) patterns.push('ENGULFING');
    ...
    // 2. Bearish engulfing
    if (c.close < c.open && c.close <= p.low && c.open >= p.high) patterns.push('ENGULFING');
...
const runStrategy = async (symbol) => {
  ...
  try {
    ...
    if (!D4_pass) {
      ...
      logDiag({ symbol, barTime, price, atr: atr.toFixed(4), D4_pass, A1_pass: false, fired: false, reason: 'D4_OVER_EXTENDED' });
      return;
    }
...
console.log('✅ MVS — Monthly Value Sniper v7.0 Started');
