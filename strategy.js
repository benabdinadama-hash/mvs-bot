 ...
 *  │  4H  → Zone cross-check (1H entry must align with 4H structure)    │
 *  │  1H  → Entry engine (confluence + rejection + absorption + SL/TP)  │
 *  └─────────────────────────────────────────────────────────────────────┘
 *
 *  v8.0 CHANGES FROM v7.2:
 *  ✅ EMA removed entirely — zero lagging indicators
 *  ✅ VAH added — full value area now: POC + VAH + VAL
 *  ✅ 4H zone cross-check — 1H entry price must sit near a 4H structural level
...
const calcVolumeProfile = (data, rows = config.VP_ROWS) => {
  ...
  while (cumVol < targetVol && (loIdx > 0 || hiIdx < rows - 1)) {
    const addLow  = loIdx > 0           ? volArr[loIdx - 1] : 0;
    const addHigh = hiIdx < rows - 1    ? volArr[hiIdx + 1] : 0;
    if (addHigh >= addLow) { hiIdx++; cumVol += addHigh; }
    else                   { loIdx--; cumVol += addLow;  }
  }
...
const get4HBias = async (symbol) => {
  ...
  // Fib 50% on 4H using dedicated lookback (BIAS_FIB_LOOKBACK bars = same calendar window as 1H)
...
//
//  After the bias confirms direction, this checks whether the 1H entry price
//  actually sits near a 4H structural level (POC, VAH, VAL, or Fib 61.8/78.6).
//
//  This is the multi-timeframe confluence layer: when the 1H entry zone
//  overlaps with a 4H structural level, both timeframes are pointing to the
//  same price — the highest-conviction setup possible.
//
//  Tolerance: 1H ATR × HTFZONE_ATR_MULT (uses entry-TF volatility for scale).
//
...
const runStrategy = async (symbol) => {
  ...
  try {
    ...
    if (data.length < 50) {
      console.log(`  ⚠️ Insufficient 1H data (${data.length} bars). Skipping.`);
      return;
    }
    ...
    // ── STEP 3: 1H FIBONACCI (200-bar swing) ────────────────────────────
    ...
    // A2: Structural remap — price broke the 1H 200-bar swing
    ...
    // 4H bias must agree with 1H direction.
    if (bias4h) {
      const biasAligned =
    ...
    // ── STEP 7: 1H VOLUME PROFILE — POC + VAH + VAL ─────────────────────
    ...
    console.log(`  🎯 1H Fib zone: $${fib.zoneLow.toFixed(2)} – $${fib.zoneHigh.toFixed(2)} (60–80% pocket)`);
    ...
    // ── STEP 8: 1H CONFLUENCE CHECK (Fib × POC / VAH / VAL) ─────────────
    ...
    // Is the 1H entry price near a 4H structural level?
    // This is the highest-conviction filter in the entire bot.
    const htfCheck = check4HZoneAlignment(bestFibLevel, bias4h, atr);
    if (!htfCheck.aligned) {
      console.log(`  ⛔ 4H ZONE MISMATCH: 1H entry $${bestFibLevel.toFixed(2)} not near any 4H structural level (nearest: ${htfCheck.nearestLevel} $${htfCheck.nearestPrice.toFixed(2)}, dist $${htfCheck.distance.toFixed(2)}). Waiting.`);
      logDiag({
        symbol, barTime, price, atr: atr.toFixed(4),
    ...
    console.log(`  ✅ 4H ZONE ALIGNED: 1H entry near ${htfCheck.nearestLevel} ($${htfCheck.nearestPrice.toFixed(2)})`);
...
💵 *Entry:* $${entryPrice.toFixed(2)} (Fib ${fibPct} ↔ 1H ${bestPivot.name})
...
⚡ *MVS v8.0 — Structure is everything.*
    `.trim();
...
console.log('');
console.log('╔══════════════════════════════════════════════════════════════╗');
console.log('║   MVS — Monthly Value Sniper v8.0   by Abdin               ║');
...
console.log(`   VP bars : ${config.VP_LOOKBACK} (1H) | ${config.BIAS_LOOKBACK} (4H)`);
console.log(`   Fib bars: ${config.FIB_LOOKBACK} (1H) | ${config.BIAS_FIB_LOOKBACK} (4H)`);
