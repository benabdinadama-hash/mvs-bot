...
// ═══════════════════════════════════════════════════════════════════════
//  STATE & LOG PERSISTENCE (committed back to repo by GitHub Actions)
//  state.json        → latest snapshot per symbol (for /status, /health)
//  signals.log.json   → append-only history of every signal (for /weekly)
// ═══════════════════════════════════════════════════════════════════════
...
const runStrategy = async (symbol) => {
  ...
  try {
    ...
    if (currentPrice > swing.high || currentPrice < swing.low) {
      ...
      saveState(symbol, { signal: 'A2_REMAP', price: currentPrice });
      logSignal(symbol, { signal: 'A2_REMAP', price: currentPrice });
    ...
    // Baseline state save — gets overwritten below if a more specific
    // signal fires, but guarantees /status always has fresh data even
    // when nothing actionable happened this scan.
    saveState(symbol, {
      signal: 'SCANNED',
      price: currentPrice,
      poc: vp.pocPrice,
    ...
    saveState(symbol, {
      signal: 'A1_GOLDEN_ZONE', price: currentPrice, entryPrice,
      confluentLevel, confluentPivot, poc: vp.pocPrice, val: vp.valPrice
    });
    logSignal(symbol, { signal: 'A1_GOLDEN_ZONE', entryPrice, confluentLevel, confluentPivot });
    ...
    if (!rejection.valid) {
      if (rejection.type === 'D1_ABSORPTION') {
        ...
        logSignal(symbol, { signal: 'D1_ABSORPTION', entryPrice });
        ...
        logSignal(symbol, { signal: 'D2_SHARP_BREAKOUT', entryPrice });
...
⚡ *MVS — "If it doesn't reject the monthly anchor, it's not a trade."*
    ...
    saveState(symbol, {
      signal: signalType, direction, entryPrice, tpPrice, slPrice, rr,
      rejection: rejection.type
    });
    logSignal(symbol, { signal: signalType, direction, entryPrice, tpPrice, slPrice, rr });
