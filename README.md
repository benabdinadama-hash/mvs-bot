...
- **(v10.32) Requested assessment: does the backtest reflect the live system, and what would improve R:R/win rate/frequency without changing the strategy's character.** Answer confirmed three real, quantifiable gaps and fixed what was safely fixable without touching the (working, historically-tuned) signal-detection logic itself. **(1) Live sizing never used the confidence weighting it already computes.** strategy.js has computed a per-signal `riskMult` (pivot/pattern/TD9 confidence, via core.js `computeRiskMultiplier`/`computePOCQualityMultiplier`) since v10.8/v10.9 and shown it in the Telegram alert as "Suggested size: X% of normal" — but `execute-signal.js` never read it; every trade executed at full $1.5 margin regardless. backtest.js's $ P&L simulation DOES apply this multiplier, so this was also a live/backtest fidelity gap, not just a missed optimization. Wired `riskMult` through `watcher.js` → `execute-signal.js`, scaling margin (and therefore notional/qty/fee-adjusted netRR) by it, with a defensive [0.1, 1.0] clamp independent of core.js's own clamp for any malformed/older-format signal record. Costs zero trade frequency — every signal still fires and executes, just sized by its own already-computed, already-vetted confidence. **Deliberately did NOT enable `VOTE_STRENGTH_SIZE_ENABLED`** despite a fresh backtest's vote-tally win-rate split (78.9% at 3-of-5 vs 100% at 4/5-of-5) looking like an obvious case for it — found on closer reading of this file's own v10.15.1 entry that this exact change was already tried and explicitly reverted: it cut simulated return roughly in half (confirmed on an isolated 47-trade backtest) because 3-of-5 tallies are 87% of all real signals, so discounting them by 30% just shrinks the vast majority of positive-EV trades rather than removing a genuinely bad segment. Recommending it without having read that history would have reintroduced an already-disproven regression. **(2) Backtest's $ P&L simulation had zero exchange fees.** Added a second, separate "LIVE-EQUIVALENT $ SIMULATION" section to backtest.js's report — same closed trades, same rr/riskMult, but run through the actual `execution/leverage.js`/`execution/fee-estimate.js` math live itself uses (fixed non-compounding $1.5 margin, real 0.055%×2 taker fee), instead of the existing GROWTH-oriented $1000/1.5%-risk/compounding/no-fees simulation, which is now clearly labeled as NOT representative of what this account's real trades did or will do. New config.js `LIVE_STARTING_BALANCE_ILLUSTRATIVE` controls only the reported return % (live sizing is fixed, not equity-scaled, so it doesn't change any individual trade's $ P&L). **(3) Signals/SL/TP levels are computed on KuCoin SPOT candles; execution happens on Bybit LINEAR PERPETUAL FUTURES** — confirmed via config.js `BASE_URL` and bybit-client.js's `category:'linear'` — a real, previously-unmeasured cross-venue/cross-instrument-type basis risk, most relevant for lower-liquidity alts. The existing staleness checks (`SIGNAL_STALE_PAST_SL`/`SIGNAL_STALE_PRICE_MOVED`) already catch severe cases functionally, but nothing measured typical basis size. Rather than restructure the proven, months-tuned signal engine onto a different data source (high risk, uncertain payoff), added plain observability: `execute-signal.js` now logs and records the KuCoin-theoretical-vs-Bybit-live basis % on every execution, turning this from a theoretical concern into reviewable data. **Also added `config.js` `TP2_MIN_EXTENSION_RR_MATRIX`** (OFF by default, mirrors the existing `SL_ATR_MULT_MATRIX` pattern exactly) — DOT-USDT and TRX-USDT both show real trigger activity in the funnel diagnostics but zero opened trades, 100% dying at the `tp2RangeOk` gate; this is a testable hypothesis (0.15 vs the global 0.25, explicitly an untested first guess, not a tuned value) for the user to A/B via `node backtest.js`, not a live change. **Scheduling**: confirmed via live logs that v10.31's phase offset alone couldn't fully eliminate mvs-scan.yml's collision — a ~4.5 min typical run + the 5 min duplicate-guard needs ~9.5 min of separation, but two triggers in a 15-min cycle can never be more than 7.5 min apart. Chose to reduce the native backup's frequency to once/hour (still at :07) rather than shrink the guard (weakens real double-write protection) or speed up the scan pipeline (real engineering effort, unjustified risk to a live 20-symbol pipeline for a cosmetic gain) — cuts collision exposure 75% for a bounded, deliberately-accepted cost: slower fallback only if cron-job.org itself ever fails, which has never actually happened in this system's history (unlike the Termux watcher, which has, repeatedly, and whose detection via the heartbeat-check step is completely unaffected by this change since that step runs on every trigger from either source). Left `mvs-commands.yml`'s schedule untouched — its v10.31 offset already fully avoids landing on a primary-cadence mark (3 is never a multiple of 5), and there's no log evidence of a residual issue there the way there was for scan.yml.**
- **(v10.31) Confirmed from live run logs: mvs-scan.yml's GitHub-native backup schedule (`*/15 * * * *`) was landing on the exact same minute as cron-job.org's primary ping often enough to trip the duplicate-run guard regularly — checkout/Node-setup/npm-install still ran and got thrown away every time (no $ cost, this repo has unlimited free Actions minutes on a public repo, but still pointless work). Root cause: cron-job.org's simplest "every 15 min" preset lands on the same four standard marks (:00/:15/:30/:45) that `*/15 * * * *` does. Offset to `7,22,37,52 * * * *` — the midpoint of each 15-min window — so the backup trigger no longer competes with the primary for the same instant, and as a side benefit now checks a different point in each window instead of re-checking the exact instant the primary already checks. **Found and fixed the same, worse-in-practice issue in `mvs-commands.yml`** while addressing this: its backup was `*/10 * * * *` against a 5-min primary — since 10 is a multiple of 5, EVERY backup firing landed on a primary mark, not just sometimes. Offset to `3,13,23,33,43,53 * * * *`, which (3 not being a multiple of 5) never lands on a primary mark at all. Neither `mvs-weekly.yml`, `mvs-backtest.yml`, nor `mvs-setup.yml` have this dual-trigger pattern, so none needed a change.**
- **(v10.30) Three fixes from a direct real-money bug report: "signal fired on Telegram but Bybit didn't pick it up," "the R:R is very bad for the SL," and a request to confirm Termux/GitHub Actions is still the right platform.** (1) **Watcher-down blind spot, confirmed real:** the Termux watcher genuinely was not running at all on 2026-09-10 — GitHub Actions kept scanning and alerting Telegram exactly as designed, but nothing on the execution side ever noticed, because the only heartbeat that existed (`execution/heartbeat.json`) is gitignored and lives ONLY on the phone, invisible to anything that isn't physically checking it. `watcher.js` now also pushes a small, git-tracked `execution/remote-heartbeat.json` periodically (`execution/heartbeat-config.js`), and a new `execution/check-remote-heartbeat.js` runs every scan (`mvs-scan.yml`) to Telegram-alert on the ok→stale transition (and again on recovery) — so a dead watcher is caught within one scan-cycle's margin of the staleness threshold, not by manually running `check-status.js` after the fact and placing the trade by hand. (2) **Fee-blind R:R, confirmed structural, not cosmetic:** every alert's R:R has always been a pure price-distance ratio with zero allowance for Bybit's own taker fees (0.055%, confirmed current rate). At a full-size account that's a rounding error; at this account's $1.5 margin/trade, round-trip fees are a fixed dollar cost that can eat a large share of a 1.2R TP1 target — worked example in `execution/fee-estimate.js`: a 0.3%-away SL turns a clean 1.2:1 gross R:R into ~0.6:1 net. New shared module `execution/fee-estimate.js` computes this once; `strategy.js`'s Telegram alert now shows it plainly next to the gross number, and `execute-signal.js` refuses to execute outright (`NET_RR_TOO_LOW_AFTER_FEES`) when it falls below `MIN_NET_RR_AFTER_FEES` (0.6, deliberately permissive — tune in one place, not two, if you want it pickier). (3) **Missing close notification, found while fixing the above:** a normal SL/TP close reconciling in `protect.js`'s `reconcileLedger()` — the ordinary, most-common case — has never once sent a Telegram message, only logged to a console nobody's watching on a phone; every other alert in this file (self-heal, circuit-breaker) already did. Now sends one, with the real Bybit-confirmed PnL, exactly once per close (can't re-fire — `recordClosed()` moves the entry out of `getOpen()`'s results for good). On the platform question: Termux + GitHub Actions is not the constraint here — every bug above was a logic/visibility gap in this codebase, not a platform limitation; a dedicated always-on server would remove the "phone has to stay alive" factor entirely, but wasn't the root cause of any of these three, so wasn't required to fix them.**
- **(v10.29) Found the actual root cause of the ledger wiping itself — and a second, related bug causing the same signal to re-fire for 11+ hours straight.** v10.28 added a defense-in-depth check after the *cause* of the ledger going empty couldn't be pinned down; this time it was found precisely. `position-ledger.js`'s `load()` treated ANY read/parse failure (not just a missing file) the same as "start fresh" and silently returned `[]`. `clearPendingClose()` — called on every reconciliation cycle for every genuinely-open position, the normal case, most cycles — then called `save()` unconditionally, even when nothing needed clearing. Together: any cycle where the file was transiently unreadable (exactly what an interrupted git pull, confirmed happening repeatedly in the watcher log both times this occurred, would cause mid-write) silently fabricated an empty ledger and wrote it straight back, permanently erasing the real one. Fixed both halves: `load()` now only treats a genuinely missing file as "start fresh" — an existing-but-unreadable file throws, and every function (read or write) has an explicit, logged, safety-first fallback (`countOpen`/`isSymbolOpen` fail toward blocking new trades; `getOpen`/`getRecentClosed` fail toward "nothing to reconcile this cycle," safe since no write follows; every write function refuses to write at all rather than save a fabricated empty state). `clearPendingClose`/`markPendingClose` also now skip the write entirely when there's nothing to actually change — closing the main amplifier, a write on literally every healthy cycle for no reason. Separately, confirmed why BTC-USDT re-fired the identical setup roughly a dozen times over 11+ hours (2026-08-31 into 09-01, the user's own words: "worsening with repetitive btc signals"): `open-positions.json` — the file that's supposed to suppress re-alerting a setup that's already open — is written by BOTH GitHub Actions (`strategy.js`, the instant a signal fires) and this phone (`protect.js`, when a position closes), a genuine cross-machine race. `pullLatest()`'s success/failure used to be silently discarded, so a cycle where the pull failed proceeded anyway on a local copy that could already be missing an entry GitHub Actions had just committed; if `syncOrphanedSignals()` then wrote that stale copy back (even just to remove some unrelated, genuinely-closed symbol) and pushed it, it silently carried the missing entry's absence back up to the shared remote — erasing the very marker meant to stop the same setup firing again. `pullLatest()` now returns whether the pull actually succeeded, threaded through `runCycle()` → `runProtectionCycle()` → `syncOrphanedSignals()`, which skips its write-back entirely on a cycle where local data can't be trusted as current. Tested by reproducing the exact confirmed corruption scenario end to end (real open position → file corrupted mid-cycle → `clearPendingClose` called, the normal case → confirmed the real position survives untouched, and is fully intact again the moment the file is readable); confirmed `clearPendingClose` is now a true no-op (zero writes, checked via file mtime) when there's nothing to clear; reproduced the BTC race directly (a signal "open" in `open-positions.json`, a failed-pull cycle → confirmed preserved; a successful-pull cycle → confirmed correctly cleaned up); re-ran the full v10.25–v10.28 regression suite to confirm every prior fix still works correctly alongside these two.
- **(v10.28) Confirmed the local ledger can genuinely diverge from reality — added a defense-in-depth check that no longer trusts it alone.** `execution/our-positions.json` went from correctly showing an open TRXUSDT entry to a fully empty array between two `check-status.js` runs, while the real Bybit position was (and still is) genuinely open the entire time — confirmed directly against live screenshots of the actual position. Traced through `reconcileLedger()` and `watcher.js`'s pull-conflict recovery in detail: neither one actually deletes an entry from the ledger array (they only ever change its `status` field), so the exact mechanism that produced a fully empty array isn't pinned down with certainty — rather than guess at one specific cause and patch only that, this closes the consequence directly regardless of cause. `execute-signal.js` now checks Bybit's own live position list as a second, independent gate right after the local ledger check — if the exchange shows a real open position for that symbol, the trade is refused even if `isSymbolOpen()` said no, and a loud `LEDGER MISMATCH` warning is logged so a recurrence is visible immediately instead of silently risking a duplicate order. A failed exchange check also refuses to execute (stricter than the ledger check, never looser) rather than proceeding on missing information. Verified by reproducing the exact confirmed scenario (empty local ledger, Bybit showing TRXUSDT genuinely open at size=45) and confirming the trade is correctly refused with zero orders placed; verified a genuinely clean symbol with no exposure anywhere still executes normally; re-ran the full v10.25/v10.26/v10.27 regression suite to confirm every prior fix still works correctly with this new gate in front of them.
- **(v10.27) v10.26's new split-exit legs confirmed for real (44/45 TRX, exactly matching the math) — but the TP1 leg filled instantly at ~breakeven, and the exact cause traced to a real 67-minute execution delay.** A TRX-USDT signal fired at 10:45 (theoretical entry 0.3365606, TP1 0.34025) but wasn't actually executed until 11:52 — 67 minutes later, matching git-pull/DNS failures visible in the watcher log for that window. By execution time the real market price had already climbed to 0.3405, past where TP1 was calculated — so the TP1 reduce-only limit order (correctly split as designed, 44 of 89 TRX) was placed BELOW the live price and filled the instant it was submitted, capturing ~nothing. Root cause: every price level (entry sizing, TP1/TP2 targets) was taken from the signal's stale theoretical numbers with no check against what the market was actually doing by execution time — a risk that exists any time there's a meaningful gap between signal generation and execution, not just during a connectivity outage. Fixed in `execute-signal.js`: fetches a live ticker price (`bybit-client.js` gained `getTicker()`) before ever entering, and rejects the trade outright — no entry order placed at all — if (a) price has already blown through the stop-loss, or (b) less than 15% of the original theoretical distance to TP1 remains. Leverage/quantity sizing also switched from the stale theoretical `entryPrice` to this live price, since that's what the real Market order will actually fill near. Verified against the exact real TRX numbers that this specific bad trade is now rejected before entry (not after); verified a fresh, healthy signal still executes normally (no false positive); verified the SL-already-breached and borderline-15%-floor cases separately; re-ran the full v10.25/v10.26 regression suite to confirm the dedup guard and split-exit logic still work correctly with this new gate in front of them.
- **(v10.26) Real trade confirmed the bot never implemented its own advertised exit strategy — fixed.** A live NEAR-USDT trade closed in full the instant TP1 was touched (small profit banked), while price kept climbing well past TP1 toward TP2 — but Bybit showed 0 open positions immediately after, meaning nothing was left running as a runner. Traced to `execute-signal.js`: it only ever read `tp1Price` from the signal — `tp2Price` was silently dropped at TWO call sites (`strategy.js` AND `execution/watcher.js`, the one that actually executes real trades) — and placed a single Market order for the FULL quantity with ONE take-profit at TP1. The Telegram alert has always said "TP1 (exit 50%, move SL to entry)... TP2 (runner, remaining 50%)", and `config.js`'s `PARTIAL_EXIT_PCT=0.5` is exactly what `core.js`'s backtest engine has always assumed when computing this strategy's advertised win-rate/R:R — live execution just never matched either one. Fixed: `execute-signal.js` now splits qty into two legs (matching `PARTIAL_EXIT_PCT`) and places TP1/TP2 as separate reduce-only limit orders (`bybit-client.js` gained `placeReduceOnlyLimit()`) instead of one TP on the entry order. The entry's stop-loss stays attached to the whole position and — confirmed against Bybit's own docs before relying on it ("Once the order is fully or partially filled, the TP/SL order will be placed for the entire position") — automatically keeps protecting whatever quantity remains after TP1 fills, no extra code needed for that part. Falls back cleanly to the old single-TP behavior when a position is too small to legally split into two exchange-minimum order sizes (routine at this account's $1.5 margin for coarser-lot-size symbols like BTC/ETH) — verified with a real too-small-to-split case that it falls back correctly rather than attempting an order Bybit would reject. Also verified the failure-resilience path: if one TP leg fails to place after the entry succeeds, the position is still correctly recorded as open (it genuinely is) with the failure surfaced, not silently lost. **Not included in this fix, deliberately scoped out:** automatically moving the stop-loss to breakeven when TP1 fills — the alert's "move SL to entry" language. The remaining leg is still fully protected by the original stop the whole time, just not as tightly as the ideal breakeven version — a real future enhancement, not a safety gap.
- **(v10.25) Double-execution structural gap found and closed, before it ever fired in production.** `strategy.js` runs on BOTH GitHub Actions (`mvs-scan.yml`, every 15 min) and is what `execution/watcher.js` effectively re-triggers execution for via its own independent poll of `signals.log.json` on Termux — two paths that could both call `executeSignal()` for the same fired signal, with no shared check between them (`watcher.js`'s dedup tracker only remembers signals it personally executed; `executeSignal()`'s only safety check was a total open-trade COUNT, never "is this exact symbol already open"). Safe so far only because Bybit's IP block against GitHub Actions has held — not a guarantee against a large, rotating cloud IP range. Two fixes, tested against mocked execution paths (no live calls) before shipping: (1) `strategy.js` now skips its inline `executeSignal()` call entirely when `process.env.GITHUB_ACTIONS === 'true'` (confirmed via GitHub's own docs: always `true` on every hosted-runner job, exactly the documented use case) — `watcher.js` on Termux is now the sole execution path, matching this repo's own documented Phase 3/4 design intent; (2) `position-ledger.js` gained `isSymbolOpen()`, checked in `execute-signal.js` as a backstop independent of cause — verified end-to-end with mocked Bybit calls that a second `executeSignal()` call for a symbol already in the ledger is rejected (`SYMBOL_ALREADY_OPEN`) before any order is placed, regardless of which two processes triggered it.
