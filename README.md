...
```
╔══════════════════════════════════════════════════════════════╗
║   MVS — Monthly Value Sniper v8.1   by Abdin               ║
║   Foundation: POC + VAH + VAL + FIBO  |  No lagging data   ║
╚══════════════════════════════════════════════════════════════╝
   Assets  : BTC-USDT, ETH-USDT, SOL-USDT, XRP-USDT, BNB-USDT, AVAX-USDT, LINK-USDT, ADA-USDT
...
## Why 15min Entry + 8 Symbols?

| Parameter | Value | Rationale |
|-----------|-------|-----------|
| **Entry Timeframe** | 15min | Finer granularity than 1H, same structure — more legitimate touches of the same daily zones, not weaker filtering. |
| **Symbols** | 8 liquid pairs | The 4H bias + zone cross-check + 2-of-4 rejection gates are unchanged per symbol — 8x the pairs = 8x the chances for the SAME strict setup to occur. |
...
| **Scan Frequency** | 15 minutes | Matches the entry timeframe — catches every fresh entry-TF candle close. |
...
> Config tracks 8 liquid pairs (`SYMBOLS` in `config.js`) on a 15min entry timeframe with the 4H bias gate + 4H zone cross-check both unchanged. More opportunities, same accuracy bar — this is the intended **2–3 signals/day** configuration.

| Signal Type | Across all 8 symbols |
|-------------|----------------------|
| A1 Golden Zone (confluence found) | ~25–40/day |
| D5/D6 Blocked (HTF disagreement/no zone match) | majority of A1 hits |
| B1/B2 Sniper Entries | **~2–3/day** |
...
├── strategy.js             # MVS v8.1 engine — scans, calculates, alerts
