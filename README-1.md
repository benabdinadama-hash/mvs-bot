...
## Operational Security — Read This Before Sharing This Repo Further

This project handles real funds via an API key with trade permissions. A few concrete precautions, not abstract advice:

**1. Make this repo private.** It's currently public. That was a reasonable default while this was a signal-only tool — it stops making sense once real execution logic, real leverage/sizing decisions, and a documented operational history (bugs hit, error codes, how the kill switch works) are sitting in it for anyone to read. Making it private costs nothing (GitHub private repos are free) and immediately reduces what a would-be attacker — or a competitor, if this is ever sold — can learn without asking you directly. GitHub → repo → Settings → scroll to "Danger Zone" → Change visibility → Private.

**2. Never zip and share `execution/.env.sh` or `execution/logs/`.** These are correctly excluded from git (`.gitignore`), but a plain folder zip doesn't respect `.gitignore` — it includes everything sitting on disk, gitignored or not. This has already happened more than once in this project's history. Before zipping the repo folder to share with anyone (including for AI-assisted debugging), delete or move these out first:
```
rm -f execution/.env.sh execution/.env.ps1
rm -rf execution/logs execution/watcher.lock
```

**3. Be skeptical of anything claiming to be Bybit/Telegram/GitHub asking you to act urgently.** The more detailed and public the operational history of a real-money bot is, the more convincing a targeted phishing attempt can be crafted (e.g., a fake "your API key expires today, renew here" message). The real renewal flow only ever happens by *you* opening the Bybit app directly — never through a link in a message, email, or DM, regardless of how official it looks.

**4. Enable 2FA on GitHub itself**, not just Bybit and Telegram. Someone with write access to this repo could alter execution logic directly (e.g., change the destination of a withdrawal-adjacent setting, though withdrawal is already disabled at the API key level as a second layer of defense).

**5. If you're considering selling this** (bot, alerts, auto-trade) to a buyer later: keeping it private now protects that option rather than closing it off — a buyer paying for something anyone could already clone for free isn't much of a sale. When that conversation becomes real, share access selectively (a private demo, a walkthrough, controlled repo access) rather than defaulting back to public. Worth flagging honestly too: marketing this as a signals/auto-trade product to other people is a different undertaking from running it yourself — similar considerations to the "managing money for family" conversation from earlier apply here too (representations about performance, and depending on how it's positioned, potential regulatory questions worth understanding rather than assuming away).
